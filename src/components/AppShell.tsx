...
import {
  ...
  Bell, ChevronDown, LogOut, Menu, X, Database, FileText, Shield
} from "lucide-react";
...
const NAV_SECTIONS: NavSection[] = [
  {
    ...
    items: [
      ...
      { label: "Products", icon: <Package size={16} />, href: "/?view=products" },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Validation", icon: <Shield size={16} />, href: "/?view=validation" },
...
const BOTTOM_NAV: NavItem[] = [
  { label: "Settings", icon: <Settings size={16} />, href: "/?view=settings" },
...
export default function AppShell({ children }: { children: ReactNode }) {
  ...
  const isActive = (href: string) => {
    ...
    return window?.location?.search?.includes(href.split("?")[1] || "") || false;
  };
    ...
    <div className="min-h-screen flex" style={{ background: "var(--bg-page)" }}>
      ...
      <aside className={`fixed lg:sticky top-0 z-50 h-screen w-[240px] flex flex-col border-r transition-transform duration-200 lg:translate-x-0 shrink-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
        ...
        <div className="flex items-center gap-2.5 px-4 h-14 border-b shrink-0" style={{ borderColor: "var(--border-default)" }}>
          <div className="flex h-7 w-7 items-center justify-center rounded-md text-white text-[10px] font-bold" style={{ background: "var(--accent-primary)" }}>PP</div>
          <div className="flex-1 min-w-0">
            <div className="text-[13px] font-semibold truncate" style={{ color: "var(--text-primary)" }}>ProductPilot</div>
            <div className="text-[10px]" style={{ color: "var(--text-muted)" }}>Enterprise Intelligence</div>
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* ── Topbar ── */}
        <header className="sticky top-0 z-40 h-14 flex items-center px-4 lg:px-6 gap-4 border-b shrink-0"
          ...
          <div className="flex items-center gap-1">
            {/* Notifications */}
            <button className="relative p-2 rounded-lg transition" style={{ color: "var(--text-muted)" }}
              onMouseEnter={(e) => e.currentTarget.style.background = "var(--bg-hover)"}
              onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
              <Bell size={16} />
            ...
            <div className="relative">
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-1 w-56 rounded-xl border shadow-lg py-1 z-50"
                  ...
                  <Link href="/?view=settings" className="flex items-center gap-2 px-3 py-2 text-[13px] transition"
                    style={{ color: "var(--text-secondary)" }}
                    onMouseEnter={(e) => e.currentTarget.style.background = "var(--bg-hover)"}
                    onMouseLeave={(e) => e.currentTarget.style.background = "transparent"}>
